<template>
    <fieldset>
      <table class="materiales_add" style="width: 100% !important;">          
        <tr>
          <th class="th-mat">#</th>
          <th class="th-mat">BUSCAR TAPACANTOS <input v-model="search_material" placeholder="Ejmp: Melamina" name="material_name"><img v-show="loader" :src=base64_loader /></th>
        </tr>
        <tr v-show="!tapacantos.length">
            <th colspan="2" class="no-material">No existen Resultados para {{search_material}}</th>
        </tr>
        <tr v-for="(tapacanto, id) in tapacantos_search">
          <td>{{ id + 1  }}</td>
          <td class="material-input">
            <label>{{ tapacanto.nombre +' '+ tapacanto.material }}
              <input type="checkbox" v-on:change="onChangeTapacantos" :value=tapacanto  v-model="tapacantos_add">
            </label>
          </td>
        </tr>
      </table>      
...
export default {
  ...
  computed: {
    tapacantos_search () {
      return this.tapacantos.filter(mat => mat.material.toLowerCase().indexOf(this.search_material.toLowerCase()) > -1)
    }
  },
  methods: {
    onChangeTapacantos: function () {
    ...
    getMateriales () {
      ...
      HTTP.get('/api/materiales/materials_for_type/T', {
        ...
        this.tapacantos = result.data.materiales
        JSON.parse(this.$store.state.general.tapacantos_add).forEach((m) => {
          this.tapacantos_add.push(this.tapacantos.find((mat) => mat.id === m.id))
        })
        self.loader = false
      }).catch(result => {
        console.log(result)
