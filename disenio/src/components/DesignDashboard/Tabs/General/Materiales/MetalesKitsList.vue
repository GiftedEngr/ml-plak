<template>
    <fieldset>
      <table class="materiales_add" style="width: 100% !important;">          
        <tr>
          <th class="th-mat">#</th>
          <th class="th-mat">BUSCAR METALES KITS <input v-model="search_material" placeholder="Ingrese Busqueda" name="material_name"><img v-show="loader" :src=base64_loader /></th>
        </tr>
        <tr v-show="!metales.length">
            <th colspan="2" class="no-material">No existen Resultados para {{search_material}}</th>
        </tr>
        <tr v-for="(metal, id) in metales_search">
          <td>{{ id + 1  }}</td>
          <td class="material-input">
            <label>{{ metal.material }}
              <input type="checkbox" v-on:change="onChangeMetales" :value=metal  v-model="metales_add">
            </label>
          </td>
        </tr>
      </table>      
...
export default {
  ...
  computed: {
    metales_search () {
      return this.metales.filter(mat => mat.material.toLowerCase().indexOf(this.search_material.toLowerCase()) > -1)
    }
  },
  methods: {
    onChangeMetales: function () {
    ...
    getMateriales () {
      ...
      HTTP.get('/api/materiales/materials_for_type/M', {
        ...
        this.metales = result.data.materiales
        JSON.parse(this.$store.state.general.metales_add).forEach((m) => {
          this.metales_add.push(this.metales.find((mat) => mat.id === m.id))
        })
        self.loader = false
      }).catch(result => {
        console.log(result)
