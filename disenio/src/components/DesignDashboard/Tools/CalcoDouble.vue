    ...
    <div v-for="(calcoInfo, index) in calcoList" class="container-calco" v-bind:class="{'division': calcoInfo.separator.division > 0, 'riel': calcoInfo.separator.elementType === 'riel', 'puerta-corrediza': calcoInfo.separator.elementType === 'puerta-corrediza'}" :data-id="calcoInfo.separator.id" :key="index">
      <table v-if="calcoInfo.separator && !calcoInfo.separator.virtual" class="table table-bordered table-calco nowrap" >
        <tbody>
          ...
          <tr>
            <td rowspan="9" @click="selectTapacantoLado(index, calcoInfo, 'derecho')" class="short-1 tapacanto">
            ...
            <td rowspan="9" @click="selectTapacantoLado(index, calcoInfo, 'izquierdo')" class="short-1 tapacanto">
          ...
          <tr>
            ...
            <td colspan="6" v-else-if="calcoInfo.separator.cajon">{{getCajonMaterial(calcoInfo)}}</td>
            <td colspan="6" v-else-if="calcoInfo.separator.riel" :class="{ grey: !!calcoInfo.extra }">{{calcoInfo.extra}}</td>
            <td colspan="6" v-else-if="calcoInfo.separator.puerta && calcoInfo.separator.elementType !== 'separador'" :class="{ grey: !!calcoInfo.extra }">{{calcoInfo.separator.puerta.material}}</td>
          ...
          <tr>
            <td colspan="6" @click="selectTapacantoLado(index, calcoInfo, 'inferior')" class="text-center short-6">
...
export default {
  ...
  mounted(){
    // let ET = Vue.extend(EditTapacantos);
    // this.ETIntance = new ET({store: this.$store, parent: this.$parent});
    // console.log(this.ETIntance);
  },
  computed: {
    dobledondo() {
