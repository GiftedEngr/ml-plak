...
class AuditoriaController extends Controller
{
	...
	public function get_by_usuario_fecha(Request $request)
	{
		...
		if (($data['area'] === 'Modulo') || ($data['area'] === 'Cajon')) {
			$records = $records->join('modulos', function($join)  {
				$join->on(function($query) {
					$query->on('modulos.id','=','audits.auditable_id')
					->where('audits.auditable_type','like', 'App%Models%Modulo');
				});
			});
			$columnas [6] = 'modulos.is_cajon';
		}
		if (($data['area'] === 'Prearmado') || ($data['area'] === 'Tapacanto')) {
			$records = $records->join('tapacantos', function($join){
				$join->on('tapacantos.id','=','audits.auditable_id')
				->where('audits.auditable_type','like', 'App%Models%Tapacanto'); 
			})
			->join('piezas', function($join) {
				$join->On(function($query) {
					$query->on('piezas.id','=','tapacantos.pieza_id')
					->where('audits.auditable_type','like', 'App%Models%Tapacanto');
				 }); 
			});
			$columnas [6] = 'piezas.estado_id as pieza_estado';
			$columnas [7] = 'tapacantos.estado_id as tapacanto_estado';
		}	
		if ($data['cliente_id'] !== 0) {
			$records = $records->leftjoin('tapacantos', function($join){
				$join->on('tapacantos.id','=','audits.auditable_id')
				->where('audits.auditable_type','like', 'App%Models%Tapacanto'); 
			})
		...
		if ($data['estado'] !== '{"estado_id":9}') {
			$records = $records->where('audits.new_values', $data['estado']);
		}	
		...
		if ($data['area'] === 'Prearmado') {
			$records = $records->where('audits.auditable_type', 'like','App%Models%Tapacanto')
							   ...
							   ->where('new_values','{"estado_id":3}');
		}
		if ($data['area'] === 'Tapacanto') {
			$records = $records->where('audits.auditable_type', 'like','App%Models%Tapacanto')
			->where(function ($query) {
				$query = $query->where('piezas.estado_id','<>',3)
							->orWhere('new_values','<>','{"estado_id":3}');
			  });
		}	
		if ($data['area'] === 'Cajon') {
	...
	private function getCountByAreaType($data,$type){
		$estadisticas = Audit::query()
		->join('usuarios', 'usuarios.id', '=', 'audits.usuario_id');
		if (($type === 2) || ($type === 3)) {
			$estadisticas = $estadisticas->join('tapacantos', function($join){
				$join->on('tapacantos.id','=','audits.auditable_id')
				->where('audits.auditable_type','like', 'App%Models%Tapacanto'); 
			})
			->join('piezas', function($join) {
		...
		if (($type === 4) || ($type === 5)) {
			$estadisticas = $estadisticas->join('modulos', function($join) use($type) {
            $join->on(function($query) {
				$query->on('modulos.id','=','audits.auditable_id')
				->where('audits.auditable_type','like', 'App%Models%Modulo');
			 });
		...
		//dd("Tipo: " . $type . " | Cliente: " . $data['cliente_id']);
		if (($type === 0) && ($data['cliente_id'] !== 0)) { 
			$estadisticas = $estadisticas->leftjoin('tapacantos', function($join){
				$join->on('tapacantos.id','=','audits.auditable_id')
				->where('audits.auditable_type','like', 'App%Models%Tapacanto'); 
			})
			->leftjoin('piezas', function($join) {
		...
		if ($data['estado'] !== '{"estado_id":9}') {
			$estadisticas = $estadisticas->where('audits.new_values', $data['estado']);
		}	
		$estadisticas = $estadisticas->where(function ($query) {
			$query->where('audits.auditable_type', 'like','App%Models%Pieza')
				->orWhere('audits.auditable_type', 'like','App%Models%Tapacanto')
				->orWhere('audits.auditable_type', 'like','App%Models%Modulo');
		})
		...
		->whereDate('audits.created_at', '<=', $data['fecha_fin']);
		if (($data['usuario_id'] !== 0)  && ($data['usuario_id'] !== -1)) {
			$estadisticas = $estadisticas->where('audits.usuario_id',$data['usuario_id']);	
		}
		if (($type === 0) && ($data['cliente_id'] !== 0)) {
			$estadisticas = $estadisticas->where('clientes.id',$data['cliente_id']);	
		}
		if ($type===1) {
			//cortes
			$estadisticas = $estadisticas->where('audits.auditable_type', 'like','App%Models%Pieza');
		}
		if ($type===2) {
			//prearmados
			$estadisticas = $estadisticas->where('audits.auditable_type', 'like','App%Models%Tapacanto')
								 ->where('piezas.estado_id',3)
								 ->where('new_values','{"estado_id":3}');
		}	
		if ($type===3) {
		...
		if ($type===4) {
			//cajones
			$estadisticas = $estadisticas->where('audits.auditable_type', 'like','App%Models%Modulo')
										  ->where('modulos.is_cajon',1);
		}
		if ($type===5) {
			//modulos
		...
		if (($type === 0) && ($data['cliente_id'] !== 0)) {
			return $estadisticas->distinct()->get(['clientes.id'])->pluck('id');
		}
	...
	private function getArea($reg) {
		...
		if (Str::contains($reg->auditable_type,'Tapacanto') === true) {
			...
			if (($tapacanto !== null) && ($tapacanto->pieza !== null)) {
				if (($tapacanto->pieza->estado_id === 3) && ($tapacanto->estado_id === 3)) {
					return 'Prearmado';
				}
			}*/
			$audit = Audit::where('id', $reg->id)->first();
			$tapacanto = Tapacanto::where('id',$reg->auditable_id)->first();
			if (($audit !== null) && ($tapacanto !== null) && ($tapacanto->pieza !== null) && ($audit->new_values !== null) && (is_array($audit->new_values) === true)){
				if (($tapacanto->pieza->estado_id === 3) && array_key_exists('estado_id', $audit->new_values) && ($audit->new_values['estado_id'] === 3)) {
					return 'Prearmado';
				}
			}	
			return 'Tapacanto';
